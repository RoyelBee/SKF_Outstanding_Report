...
outstanding_df = pd.read_sql_query(""" select
                isnull(SUM(CASE WHEN TERMS='CASH' THEN OUT_NET END),0) AS TotalOutStandingOnCash,
                isnull(SUM(CASE WHEN TERMS not like '%CASH%' THEN OUT_NET END),0) AS TotalOutStandingOnCredit
...
credit_category_df = pd.read_sql_query(""" Select case when Days_Diff>0 then 'Matured Credit' else 'Regular Credit' End as  'Category',isnull(Sum(OUT_NET),0) as Amount from
...
sector_credit_df = pd.read_sql_query(""" Select case when MAINCUSTYPE='RETAIL' then 'Retail' else 'Institute' end  as CustType,isnull(Sum(OUT_NET),0) as Amount from
                    (select INVNUMBER,INVDATE,
                    CUSTOMER,TERMS,MAINCUSTYPE,
                    CustomerInformation.CREDIT_LIMIT_DAYS,
                    datediff([dd] , CONVERT (DATETIME , LTRIM(cust_out.INVDATE) , 102) , GETDATE())+1-CREDIT_LIMIT_DAYS as Days_Diff,
...
mature_credit_df = pd.read_sql_query("""
                    Select case when MAINCUSTYPE='RETAIL' then 'Retail' else 'Institute' end  as CustType,isnull(Sum(OUT_NET),0) as Amount from
                    (select INVNUMBER,INVDATE,
                    CUSTOMER,TERMS,MAINCUSTYPE,
                    CustomerInformation.CREDIT_LIMIT_DAYS,
                    datediff([dd] , CONVERT (DATETIME , LTRIM(cust_out.INVDATE) , 102) , GETDATE())+1-CREDIT_LIMIT_DAYS as Days_Diff,
...
CloseTo_mature_df = pd.read_sql_query("""
                    SELECT  AgingDays, isnull(sum(CreditAmount),0)/1000 as Amount FROM
                    (Select CUSTOMER, CUSTNAME, INVDATE,   isnull(Sum(OUT_NET),0) as CreditAmount ,
                    case
...
aging_mature_df = pd.read_sql_query("""
                     SELECT  AgingDays, isnull(sum(Amount),0)/1000 as Amount FROM
                    (Select
                    case
                    ...
                    --OUT_NET
                    isnull(Sum(OUT_NET),0) as Amount,
...
# --------------------------- Sector wise cash --------------------------
sector_wise_cash_df = pd.read_sql_query(""" Select case when MAINCUSTYPE='RETAIL' then 'Retail' else 'Institute' end  as CustType,isnull(Sum(OUT_NET),0) as Amount from
                    (select INVNUMBER,INVDATE,
                    CUSTOMER,TERMS,MAINCUSTYPE,
                    CustomerInformation.CREDIT_LIMIT_DAYS,
                    datediff([dd] , CONVERT (DATETIME , LTRIM(cust_out.INVDATE) , 102) , GETDATE())+1-CREDIT_LIMIT_DAYS as Days_Diff,
...
cash_drop_df = pd.read_sql_query(""" SELECT  AgingDays, isnull(sum(Amount),0)/1000 as Amount FROM (  Select
        case
